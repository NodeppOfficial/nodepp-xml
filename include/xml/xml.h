...
protected:
    ...
    object_t parse_tag( const string_t& str, const ptr_t<ulong>& data ) const noexcept {
        ...
        thread_local static regex_t reg3( "[\n\t ]+" ); 
        
        auto raw=str.slice_view(data[1],data[2]);
    ...
    string_t format_tag( const object_t& obj ) const {
        ...
        if(!obj.has("type")      ){ return nullptr; }
    ...
    string_t format_array( const ARRAY& obj ) const { 
        ...
        string_t tmp     = format_object(x);
        if( tmp.empty() ){ return nullptr; }
    borrow += tmp; } return borrow; }

public: xml_t() {}

    string_t format( const object_t& obj ) const { return format_object( obj ); }

    object_t parse ( const string_t& str ) const { ulong offset = 0; 
        auto tmp = parse_child( parse_list(str), offset );
        if( !tmp ){ return nullptr; } return object_t({
            { "_root_", "xml" }, { "children", tmp }
        });
    }
...
namespace nodepp { namespace xml {
    ...
    inline bool is_valid_element( object_t dom, string_t type ){
...
namespace nodepp { namespace xml {
    ...
    thread_local static regex_t reg ( "\\s+|,+" );
